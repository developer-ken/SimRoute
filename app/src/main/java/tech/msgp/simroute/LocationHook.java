...
*/
        ...
        XposedHelpers.findAndHookMethod(LocationManager.class, "getLastLocation", new XC_MethodHook() {
            @Override
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                Location location = XLocationChanger.getLocationX();
        ...
        for (Method method : LocationManager.class.getDeclaredMethods()) {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                        if (param.args.length >= 4 && (param.args[3] instanceof LocationListener)) {
                            ...
                            XposedBridge.hookMethod(m, new XC_MethodHook() {
                                @Override
                                protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                                    if((XLocationChanger.ishookenabled())) {
                                        Location location = (Location) param.args[0];
                                        XposedBridge.log("before " + location.getLatitude() + location.getLongitude());
                                        location = XLocationChanger.getLocationX();
                                        param.args[0] = location;
                                        XposedBridge.log("after " + location.getLatitude() + location.getLongitude());
                                    }
                                }
                            });
                            if((XLocationChanger.ishookenabled())) {
                                ...
                                try {
                                    if (m != null) {
                                        ...
                                        XposedBridge.log("调用onLocationChanged");
                                        XposedBridge.log("调用onLocationChanged:" + ",经度" + location.getLatitude() + ",纬度" + location.getLongitude());
                                    }
                                } catch (Exception e) {
                                    XposedBridge.log(e);
                                }
            ...
            if (method.getName().equals("requestSingleUpdate")
                ...
                XposedBridge.hookMethod(method, new XC_MethodHook() {
                    @Override
                    protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                        if (param.args.length >= 3 && (param.args[1] instanceof LocationListener)&&(XLocationChanger.ishookenabled())) {
                            ...
                            try {
                                if (m != null) {
                                    Location location = XLocationChanger.getLocationX();
                                    m.invoke(ll, location);
                                    XposedBridge.log("onLocationChanged  :"  + ",经度" +location.getLatitude() + ",纬度" +location.getLongitude());
                                }
                            } catch (Exception e) {
                                XposedBridge.log(e);
                            }
